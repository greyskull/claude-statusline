...
- Identity row: `<elapsed> agent_type · description · <lines> · <N> (<share>%) · <model> · <current-activity>` — duration-first, **no run-state marker glyph**. The right-hand cluster is `· <lines> · <N> (<share>%) · <model>`; the description fills the gap in between. In tree-single mode a `·` plus a constant gap (`SUBAGENT_STATS_ACTIVITY_GAP`) separates the cluster from the current-activity snippet, which renders on the SAME row rather than a second line.
- Continuation row (non-tree-single only): `└ <current-activity>` — activity only, no right-aligned metrics.

`<share>%` renders as a parenthetical suffix on the token figure (`<N> (<share>%)`, e.g. `7.52M (10.5%)`) rather than its own segment. In tree-single mode the token figure uses a **3-significant-figure** format (`fmt_tok_fixed` in `render/text.py` — a single-digit mantissa gets 2 decimals, e.g. `7.52M`/`3.50M`, so every reading in the cohort renders at the same width) and the `<share>%` figure right-aligns to the cohort's measured max share width (`layout.tree_share_width`) so the trailing `%` lands on the same column across rows — never applied to the session-level input/cache/output row or **Day Total**, which keep `fmt_tok`'s original 1-decimal behaviour. The `<lines>` field (this subagent's own **Lines Read**/**Lines Changed**, self-scoped) sheds its read and changed sides *independently*: a subagent that only wrote renders a blank (not `0`) on the read side while still showing the changed side, and vice versa — each blank occupies exactly the width the populated value would have, so the cluster's total width never shifts.

The line-1 right cluster sheds under width pressure in priority order: the description truncates first, then `<lines>` is dropped, then `<share>%`, then the `<N>` token count — while `<model>` and the front `<elapsed>` always render. Independently of width, `<share>%` is omitted when the session denominator is 0. The `<N>` token figure and the model are padded to the cohort's measured widths (tree-single) so the `·` separators line up across rows. The **t/m rate** and **↑output** fields are no longer rendered in any form (removed).
...
**Session Share %**:
This subagent's fraction of the whole session's token spend, computed as `sub_inout / (main_inout + Σ subagent_inout)`. Rendered as a `NN.N%` figure (one decimal) inside parentheses AFTER the token figure (`<N> (<share>%)`) in the two-line subagent row's line-1 right cluster. A `·` separator precedes the cluster. In tree-single mode the figure right-aligns to the cohort's measured max share width (`layout.tree_share_width`) so the `%` glyph lands on the same column across rows. Omitted when the session denominator is 0.
