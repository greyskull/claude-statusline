...
**Subagent Tree View**:
The opt-in nested rendering of the **Running Subagent** cohort, enabled by the `subagent_tree` **Config** knob (`YAS_SUBAGENT_TREE`, `[layout].subagent_tree`, default off — flat rendering is byte-identical when disabled). Parentage comes from the meta.json's `parentAgentId` (matched against the spawner's transcript filename stem, with or without the `agent-` prefix) with `spawnDepth` read alongside; an agent whose parent is absent or unknown renders as a top-level root. Rows reorder depth-first — children group directly under their parent, siblings keep `first_timestamp` order — and each child row carries a dim branch prefix: `├ ` for a sibling, `└ ` for its parent's last child, indented two further spaces per extra nesting level (roots carry none). Because every row's front field (prefix + duration + type) is padded to the cohort's common `desc_col`, the elbow visually lands under the parent's own type-name column — its indentation is depth-based, not a per-row measurement, but the front-padding scheme makes the two equivalent in practice. The prefix eats visible columns off the row's content width and shifts the stats anchor left by the same amount, so the box edges and the stats cluster stay aligned across mixed depths; the two-line continuation row indents under the branch. In tree mode the wide layout always stacks single-column (the paired two-column grid would tear branches apart). In **`ascii`** glyph mode both `├` and `└` (and the line-2 `GLYPH_CONTINUATION` marker, which shares the same `└` codepoint) collapse to a single `L` — ascii mode makes no sibling/last-child distinction — rather than a generic `+` corner.
_Avoid_: "nested subagents" as the knob name (the canonical term is **Subagent Tree View** / `subagent_tree`) and confusing the branch `└` with `GLYPH_CONTINUATION` (the line-2 activity marker — same glyph, different row role, and now the same ascii fallback too).
