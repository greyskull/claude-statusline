...
def test_samples_inside_one_window_anchor_at_earliest_sample(tmp_home: Path) -> None:
    _write(
        tmp_home,
        '1000 sess-a 500 0 0 0',
        '1200 sess-a 900 0 0 0',
        '1500 sess-b 300 0 0 0',
    )
...
def test_resets_at_is_stable_across_two_now_values_within_the_window(tmp_home: Path) -> None:
    _write(tmp_home, '1000 sess-a 500 0 0 0')
...
def test_now_past_first_window_advances_anchor_to_next_windows_first_sample(tmp_home: Path) -> None:
    _write(
        tmp_home,
        '1000 sess-a 500 0 0 0',            # opens window [1000, 1000+WINDOW)
        f'{1000 + WINDOW} sess-a 900 0 0 0',  # first activity of the next window
    )
...
def test_multiple_lapsed_windows_are_skipped_in_one_call(tmp_home: Path) -> None:
    _write(
        tmp_home,
        '0 sess-a 100 0 0 0',                    # window 1: [0, WINDOW)
        f'{3 * WINDOW} sess-a 200 0 0 0',         # window 4: [3*WINDOW, 4*WINDOW) -- only sample after window 1
    )
    now = 3 * WINDOW + 10
...
def test_lapsed_window_with_no_later_samples_falls_back_to_now(tmp_home: Path) -> None:
    _write(tmp_home, '0 sess-a 100 0 0 0')  # window [0, WINDOW), nothing since
...
def test_anchor_is_account_wide_not_per_session(tmp_home: Path) -> None:
    _write(
        tmp_home,
        '1000 sess-a 500 0 0 0',   # earliest overall -> opens the window
        '1400 sess-b 200 0 0 0',
    )
    now = 2000.0
